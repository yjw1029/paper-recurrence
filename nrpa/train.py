...
dir = "../data/yelp13/"
train_set = NRPADataset(dir+"train.txt", dir+"usrlist.txt", dir+"prdlist.txt", dir+"wordlist.txt")
test_set = NRPADataset(dir+"test.txt", dir+"usrlist.txt", dir+"prdlist.txt", dir+"wordlist.txt")
...
# init sequence: weights_matrix_user, weights_matrix_item, uid_matrix_len, iid_matrix_len
nrpa_para = torch.load('embed_weight.pkl')
net = NRPA(nrpa_para['weights_matrix_user'],
           nrpa_para['weights_matrix_item'],
           nrpa_para['uid_matrix_len'],
           nrpa_para['iid_matrix_len']).to(device)
...
def train_net(loader):
    ...
    for epoch in range(EPOCH):
        ...
        for step, (user_review_vec, user_id_vec, item_review_vec, item_id_vec, rate) in enumerate(loader):
            ...
            rate = rate.to(device)
            optimizer.zero_grad()
            # params_sequence: user_review_vectors, user_id_vector, item_review_vectors, item_id_vector
        ...
        torch.save(net, 'Model/NRPA{}.pkl'.format(epoch))
...
torch.save(net, 'Model/NRPA_last.pkl')
