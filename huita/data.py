...
from config import WORDS_SIZE, REVIEWS_SIZE, SENTENCES_SIZE
import nltk
import bisect
...
class HUITADataset(Dataset):
    ...
    def get_word_vec_of_sentence(self, sentence: str, vec_len=WORDS_SIZE, padding = WORD_PADDING) -> list:
        sentence_vocab_list =  nltk.word_tokenize(sentence)
        len_vocab_list = len(sentence_vocab_list)
        sentence_vector = []

        for i in range(vec_len):
            if i < len_vocab_list:
                try:
                    vocab_num = self.voc_dict[sentence_vocab_list[i]]
                except KeyError:
                    # padding to word_size
                    vocab_num = padding
            else:
                vocab_num = WORD_PADDING
            sentence_vector.append(vocab_num)

        return sentence_vector

    def get_vec_of_review(self, review: str, vec_len = SENTENCES_SIZE):
        review_sent_list =  nltk.sent_tokenize(review)
        len_sent_list = len(review_sent_list)

        review_vec = []

        for i in range(vec_len):
            if i < len_sent_list:
                sent_vec = self.get_word_vec_of_sentence(review_sent_list[i])
            else:
                # padding to sent_size
                sent_vec = self.get_word_vec_of_sentence('')
            review_vec.append(sent_vec)
        return review_vec 
    
    def get_review_tensor_onesall(self, name, isuser=True, vec_len = REVIEWS_SIZE):
        if isuser:
    ...
    def __getitem__(self, index):
        with open(self.review_file, 'r', errors="replace") as f_review:
            ...
            user_review_vectors = self.get_review_tensor_onesall(user_name)
            item_review_vectors = self.get_review_tensor_onesall(item_name, isuser=False)
